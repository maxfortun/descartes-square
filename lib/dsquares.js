...
async function appInfo(req) {
	...
	const dsquareCountCursor = await mongoose.connection.collection('dsquares')
				.aggregate([
					{ $match: { accounts: req.account.email } },
					{ $count: 'id' }
				]);

	for await (const dsquareCount of dsquareCountCursor) {
		debug(req.id, "dsquareCountCursor", dsquareCount);
		appInfo.counts.squares = dsquareCount.id;
	}
