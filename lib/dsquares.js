...
async function openaiCompletion(req) {
	const square = await findSquare(req);
	const content = `Respond to each of the following questions in JSON. What will happen if I do ${square.decision}? What will happen if I do not ${square.decision}? What will not happen if I do ${square.decision}? What will not happen if I do not ${square.decision}?`;

	debug(req.id, 'openai >', content);
	...
	debug(req.id, 'openai <', completion);
